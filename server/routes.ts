...
export async function registerRoutes(httpServer: Server, app: Express): Promise<void> {
  ...
  app.post("/api/jobs/sync-goalserve", requireJobSecret("GOALSERVE_SYNC_SECRET"), async (req, res) => {
    ...
    try {
      await runWithJobContext(run.id, async () => {
        ...
        const result = await runSyncGoalserve(leagueId, seasonKeyParam, run.id);
  ...
  app.post("/api/jobs/backfill-pamedia-hero-images", requireJobSecret("PAMEDIA_INGEST_SECRET"), async (req, res) => {
    try {
      const result = await runBackfillPaMediaHeroImages();
      const payload = {
        ok: result.ok,
        processed: result.processed,
        updated: result.updated,
      ...
      console.error("[backfill-pamedia-hero] Job error:", err);
      res.status(500).json({
        ok: false,
        processed: 0,
        updated: 0,
  ...
  // POST /api/admin/sync/ghost - Full Ghost sync (async, non-blocking)
  app.post("/api/admin/sync/ghost", async (req, res) => {
    ...
    console.log(`[Ghost sync admin] Started at ${startTime}`);
    res.status(202).json({ ok: true, started: true, timestamp: startTime });
    
    // Run sync asynchronously in detached block
    (async () => {
      try {
        const result = await triggerGhostSync();
        console.log(`[Ghost sync admin] Completed: ${result.message} (synced: ${result.totalSynced}, errors: ${result.totalErrors})`);
      } catch (e: any) {
        console.error(`[Ghost sync admin] Failed:`, e.message || e);
      }
    })();
  });
