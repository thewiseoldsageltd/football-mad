...
# Football Mad – Day 4 Decision Log (ALL)

Decision 1: Treatment Room is medical-only
→ Suspended moved to Discipline, Loaned/Transferred moved to Transfers.

Decision 2: Medical states limited to four
→ 75 / 50 / 25 / 0 with exact reconciliation.

Decision 3: Colour logic simplified
→ Doubtful and Out both red; green banned from Treatment Room.

Decision 4: Overview sorted by usefulness
→ Highest availability first, not lowest.

Decision 5: Filters are modifiers, not navigation
→ Sub-nav is primary; sort dropdown renamed and clarified.

Decision 6: Mobile nav is single-line, scrollable
→ Two-line wrapped nav rejected.

Decision 7: Team filter restricted to current Premier League clubs
→ Deduped and canonicalised.

Decision 8: Flat article URLs retained
→ /news/article-slug only, no category paths.

Decision 9: News entity pages use Option A
→ /news/arsenal, /news/premier-league.

Decision 10: Entity pages are indexes, not parents
→ Articles appear in multiple contexts without duplication.

Decision 11: URL structure locked pre-launch
